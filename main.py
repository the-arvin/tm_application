...
#     """
#     Process a response from google bigquery and return a dictionary with 
#     start and end values.

#     Args:
#         response: A list of response data.

#     Returns:
#         A dictionary with 'start' and 'end' keys.
#     """
#     my_dict = dict()
#     for row in response:
#         my_dict['start'] = row['min']
#         my_dict['end'] = row['max']
#     return my_dict
...
def gather_bqdata(i):
    ...
    """
    return gbq_utils.query_bq_table()
...
if __name__ == '__main__':
    ...
    with st.sidebar:
        ...
        if st.button('Load, Clean, and Update Data'):
            ...
            success,error = gbq_utils.write_process(df)
    ...
    with mainA:
        ...
        date_segment = st.radio(label = 'Filter by',options = ['Day','Week','Month','Year','Overall'])

    with mainB:
        i_header = st.container()
        table_tab, charts_tab, raw_tab = st.tabs(['View All User Data in a Table','View Data in Charts','Raw and Cleaned Tables'])
        with table_tab:
            p1,cTable,p2 = st.columns([2,7,1])
            with cTable:
                df_temp,time_str = df_filter(df[df.user==warrior],date_of_interest,date_segment)
                if len(df_temp)>0:
                    st.dataframe(df_temp.reset_index(drop=True))
                else:
                    st.info(f"""Sorry dear, it seems that {warrior.title()} didn't check in for this time and segment""")    
        
        with charts_tab:
            df_agg = aggregate_project(df_temp)
            total_hours =  dec2hms(df_agg.sum())
            if sum(df_agg)>0:
                st.plotly_chart(plotly_chart(df_agg,date_segment,warrior,total_hours))
            else:
                st.info(f"""Sorry dear, it seems that {warrior.title()} didn't check in for this time and segment""")
                
        with raw_tab:
            st.info('Raw table is only going to show if Data is loaded from CSV source')
